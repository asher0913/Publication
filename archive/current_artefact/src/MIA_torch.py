"""
MIA_torch.py — alternative model-inversion attack pipeline.

Defines the `MIA` class used as a drop-in alternative to
`model_training_paral_pruning.MIA_train` for older defence baselines.
Most of the head-line-run pipeline goes through `MIA_train` instead;
this file is imported by `main_test_MIA.py` so that older checkpoints
can still be resumed through `MIA_torch.MIA(...)` if needed.

The bulk of the file is the upstream CEM attack code (Peng *et al.*,
CVPR 2025); structural changes from upstream are limited to logging
and to the wiring through to the SCA-CEM defence option.
"""
...
def init_weights(m):
    """Xavier init for linear / conv layers; zero bias if present."""
    if type(m) == nn.Linear:
...
class MIA: # main class for every thing
    ...
    def __init__(self, arch, cutting_layer, batch_size, n_epochs, scheme="V2_epoch", num_client=2, dataset="cifar10",
        ...
        if "resnet20" in arch:
            self.logger.debug("\nSplit Learning Scheme: Overall Cutting_layer {}/10".format(self.cutting_layer))
        if "vgg11" in arch:
            self.logger.debug("\nSplit Learning Scheme: Overall Cutting_layer {}/13".format(self.cutting_layer))
        if "mobilenetv2" in arch:
            self.logger.debug("\nSplit Learning Scheme: Overall Cutting_layer {}/9".format(self.cutting_layer))
    ...
    def validate_target(self, client_id=0):
        ...
        # print('the length of val_loader is:',len(val_loader),val_loader)
...
# pre-train a GAN with local data before SFL training
    def pre_GAN_train(self, num_epochs, select_client_list=[0]):
        ...
        for client_id in select_client_list:
            ...
            self.gen_ir(val_test_loader, self.model.local_list[client_id], image_data_dir, tensor_data_dir)
    ...
    def attack(self, num_epochs, local_model, decoder, optimizer,scheduler, trainloader, testloader, logger, path_dict, batch_size,
        ...
        for epoch in range(round_ * num_epochs, (round_ + 1) * num_epochs):
            ...
            for i, (input, target) in enumerate(testloader):
                ...
                rec_inputs = normalize(output)
                pred = self.model.local_list[0](rec_inputs)
                with torch.no_grad():
