"""
datasets_torch.py — dataset loaders used by the training and MIA pipelines.

Each dataset has a `get_<name>_trainloader` / `get_<name>_testloader` /
`get_<name>_bothloader` triple. The `bothloader` variant is what the
split-learning training loop calls when the attacker side
(`collude_use_public`) needs an auxiliary loader on the public split
of the same data.

The means/stds at the top are the canonical per-dataset normalisation
constants. Switching them silently breaks the MIA reconstruction
metrics, since the inversion decoder is trained on the same
normalised feature space.

Multi-client (`num_client > 1`) branches are kept here for the
federated-learning ablations. The headline FaceScrub run uses
`num_client = 1`, so most of those branches don't fire.
"""
...
def get_facescrub_bothloader(batch_size=16, num_workers=2, shuffle=True, num_client = 1, collude_use_public = False):
    ...
    Returns: train_data_loader:torch dataloader object
    """
    # FaceScrub is expected to be provided locally as an ImageFolder-style dataset.
    # Resolution is anchored to this file's location so the loader keeps working
    # whether the script is launched from the artefact root or from src/.
    #
    # Search order (first match wins):
    #   1. <artefact_root>/data/facescrub        ← intended location
    #   2. <src dir>/data/facescrub              ← legacy (data inside src/)
    #   3. ./data/facescrub                      ← legacy (relative to cwd)
    #   4. ./facescrub-dataset/{64x64,48x48,/}   ← repo-prepared subset (legacy)
    ...
    _artefact_root = os.path.dirname(_this_dir)

    candidate_roots = [
        os.path.join(_artefact_root, "data", "facescrub"),
        os.path.join(_this_dir, "data", "facescrub"),
        "./data/facescrub",
    ]
    dataset_root = _first_existing_dir(candidate_roots) or candidate_roots[0]
    ...
    # Legacy fallback: repo-prepared FaceScrub subset (preprocessing script output).
    if train_dir is None or val_dir is None:
        legacy_root = _first_existing_dir([
            os.path.join(_artefact_root, "facescrub-dataset", "64x64"),
            os.path.join(_artefact_root, "facescrub-dataset", "48x48"),
            os.path.join(_artefact_root, "facescrub-dataset"),
    ...
    if train_dir is None or val_dir is None:
        raise FileNotFoundError(
            ...
            "'<artefact_root>/data/facescrub/train' and '<artefact_root>/data/facescrub/val' "
            "(or 'validate'/'test'). Searched: {}".format(candidate_roots)
        )

    # Crucial for Slot Attention: train/eval at 64x64 resolution.
    ...
    facescrub_training_loader_AT=torch.utils.data.DataLoader(facescrub_training_AT,  batch_size=batch_size, shuffle=shuffle,
                num_workers=num_workers)
    ...
    facescrub__training_nomem = torch.utils.data.Subset(facescrub_testing, list(range(0, len(facescrub_testing)-len(facescrub_testing)//10)))
    facescrub_testing_loader_AT = DataLoader(
        facescrub__training_nomem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)

    facescrub__testing_nomem = torch.utils.data.Subset(facescrub_testing, list(range(len(facescrub_testing)-len(facescrub_testing)//10, len(facescrub_testing))))
    facescrub_testing_loader_val = DataLoader(
        facescrub__testing_nomem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)


    return facescrub_training_loader, facescrub_testing_loader,facescrub_training_loader_AT, facescrub_testing_loader_AT,facescrub_testing_loader_val
...
def get_tinyimagenet_bothloader(batch_size=16, num_workers=2, shuffle=True, num_client = 1, collude_use_public = False):
    ...
    tinyimagenet_testing = datasets.ImageFolder('tiny-imagenet-200/val/images', transform=transform_test)
    tinyimagenet_training_AT = datasets.ImageFolder('tiny-imagenet-200/train', transform=transform_test)
    ...
    if num_client == 1:
        ...
        for i in range(num_client):
            tinyimagenet_training_subset = torch.utils.data.Subset(tinyimagenet_training, list(range(i * (len(tinyimagenet_training)//num_client), (i+1) * (len(tinyimagenet_training)//num_client))))
            subset_training_loader = DataLoader(
                tinyimagenet_training_subset, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)
    ...
    tinyimagenet_training_loader_AT=torch.utils.data.DataLoader(tinyimagenet_training_AT,  batch_size=batch_size, shuffle=shuffle,
                num_workers=num_workers)

    tinyimagenet__training_nomem = torch.utils.data.Subset(tinyimagenet_testing, list(range(0, len(tinyimagenet_testing)-len(tinyimagenet_testing)//10)))
    tinyimagenet_testing_loader_AT = DataLoader(
        tinyimagenet__training_nomem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)

    tinyimagenet__testing_nomem = torch.utils.data.Subset(tinyimagenet_testing, list(range(len(tinyimagenet_testing)-len(tinyimagenet_testing)//10, len(tinyimagenet_testing))))
    tinyimagenet_testing_loader_val = DataLoader(
        tinyimagenet__testing_nomem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)                

    return tinyimagenet_training_loader, tinyimagenet_testing_loader,tinyimagenet_training_loader_AT, tinyimagenet_testing_loader_AT,tinyimagenet_testing_loader_val


def get_imagenet_bothloader(batch_size=16, num_workers=2, shuffle=True, num_client = 1, collude_use_public = False):
    """ return training dataloader
    Args:
        mean: mean of cifar10 training dataset
        std: std of cifar10 training dataset
    ...
    if not os.path.isdir("../ImageNet/train"):
        import subprocess
        subprocess.call("python prepare_tinyimagenet.py", shell=True)
    tinyimagenet_training = datasets.ImageFolder('../ImageNet/train', transform=transform_train)
    tinyimagenet_testing = datasets.ImageFolder('tiny-imagenet-200/val/images', transform=transform_test)
    tinyimagenet_training_AT = datasets.ImageFolder('tiny-imagenet-200/train', transform=transform_test)
    ...
    if num_client == 1:
        ...
        if not collude_use_public:
            ...
            for i in range(num_client-1):
                ...
                cifar10_training_loader.append(subset_training_loader)
    cifar10_training2 = torchvision.datasets.CIFAR10(root='./data', train=True, download=True, transform=transform_test)

    cifar10_training_mem = torch.utils.data.Subset(cifar10_training2, list(range(0, 50000)))
    xmem_training_loader = DataLoader(
        cifar10_training_mem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)

    cifar10_testing_mem = torch.utils.data.Subset(cifar10_training2, list(range(5000, 5032)))
...
def get_cifar10_testloader(batch_size=16, num_workers=2, shuffle=True, extra_cls_removed_dataset = False, cls_to_remove = 0):
    ...
    cifar10_test2 = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform_test)
    cifar10_training_nomem = torch.utils.data.Subset(cifar10_test2, list(range(0, len(cifar10_test2)-len(cifar10_test2)//10)))
    nomem_training_loader = DataLoader(
        cifar10_training_nomem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)

    cifar10_testing_nomem = torch.utils.data.Subset(cifar10_test2, list(range(len(cifar10_test2)-len(cifar10_test2)//10, len(cifar10_test2))))
...
def get_cifar100_trainloader(batch_size=16, num_workers=2, shuffle=True, num_client = 1, collude_use_public = False, data_portion = 1.0, noniid_ratio = 1.0):
    ...
    cifar100_training2 = torchvision.datasets.CIFAR100(root='./data', train=True, download=True, transform=transform_test)

    cifar100_training_mem = torch.utils.data.Subset(cifar100_training2, list(range(0, 50000)))
    xmem_training_loader = DataLoader(
        cifar100_training_mem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)

    cifar100_testing_mem = torch.utils.data.Subset(cifar100_training2, list(range(5000, 5032)))
    xmem_testing_loader = DataLoader(
...
def get_cifar100_testloader(batch_size=16, num_workers=2, shuffle=True, extra_cls_removed_dataset = False, cls_to_remove = 0):
    ...
    cifar100_training_nomem = torch.utils.data.Subset(cifar100_test2, list(range(0, len(cifar100_test2)-len(cifar100_test2)//10)))
    nomem_training_loader = DataLoader(
        cifar100_training_nomem, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)

    cifar100_testing_nomem = torch.utils.data.Subset(cifar100_test2, list(range(len(cifar100_test2)-len(cifar100_test2)//10, len(cifar100_test2))))
    nomem_testing_loader = DataLoader(
...
def get_celeba_trainloader(batch_size=16, num_workers=2, shuffle=True, num_client = 1, collude_use_public = False):
    ...
    celeba_training = torchvision.datasets.CelebA(root='./data',split='train', download=True, transform=transform_train)
    if num_client == 1:
