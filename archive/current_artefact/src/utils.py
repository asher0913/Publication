"""
utils.py — shared helpers used across the training and MIA pipelines.

Contents (rough grouping; see individual class/function docstrings for
details):

  * loss / regulariser helpers     — clustering_loss, prune_*_defense,
                                     dropout_defense, spurious_loss,
                                     MMD_loss, DistanceCorrelationLoss,
                                     dist_corr*, TV, l2loss, get_PSNR
  * model-state utilities          — freeze_model_bn, average_weights,
                                     zeroing_grad, save_grad, load_grad
  * training infrastructure        — setup_logger, AverageMeter,
                                     accuracy, WarmUpLR, scheduler_to,
                                     optimizer_to
  * data utilities                 — CustomPad, GaussianSmoothing,
                                     ImageTensorFolder,
                                     apply_transform, apply_transform_test
  * misc / debug                   — torch_diff, plot_change, summary,
                                     summary_string

Most of the function signatures match the upstream CEM codebase so that
diffs against the original release stay readable.
"""
...
# default formatter for setup_logger() — kept module-level so that other
# files can reuse it directly if they want to attach extra handlers.
...
def apply_transform_test(batch_size, image_data_dir, tensor_data_dir, limited_num = None, shuffle_seed = 123, dataset = None):
    """
    """
    # No normalisation here — the upstream pipeline stores tensors that
    # are already in the encoder's expected range.
    std = [1.0, 1.0, 1.0]
    mean = [0.0, 0.0, 0.0]
...
def apply_transform(batch_size, image_data_dir, tensor_data_dir, shuffle_seed = 123, dataset = None):
    """Build train/test loaders over an ImageTensorFolder.

    The mean/std are intentionally [0,1]/[1,1,1] (no normalisation) —
    upstream stores tensors that have already been pre-normalised by
    the dataset-specific loader.
    """
    std = [1.0, 1.0, 1.0]
    mean = [0.0, 0.0, 0.0]
    ...
    # Note: do NOT shuffle here — the upstream tensor folder is already
    # in the order expected by the MIA attack pipeline.
